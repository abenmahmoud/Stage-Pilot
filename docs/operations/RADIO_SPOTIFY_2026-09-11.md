...
## Ajustement de la bienvenue demandé le 11 septembre
Adel demande le retrait temporaire des seules invitations « Radio ESSUF vous accompagne, quand vous le souhaitez. » et « Découvrir la radio ». Elles sont supprimées de l’accueil de bienvenue. Le bouton principal, le crédit ESSUF, le lecteur accessible par « radio » et les autres services restent identiques. Recette navigateur ciblée sur ordinateur et téléphone ; preuves hors Git dans BRANCHEMENT_SPOTIFY_2026-09-11/welcome-cleanup-*.
Validation de cet ajustement : npm run build et npm run test:preview-security-gate réussis ; recette Chromium 1440 et 390 px réussie, captures relues.
