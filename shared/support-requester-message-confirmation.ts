export type SupportRequesterMessageConfirmation = {
  status: "stored";
  operation: "support_requester_message";
  publicCode: string;
  messageId: string;
  duplicate: boolean;
...
export function createSupportRequesterMessageConfirmation(input: {
...
}): SupportRequesterMessageConfirmation {
  if (
    ...
    throw new Error("Support requester message confirmation is invalid");
  }
  ...
  return {
    ...
    confirmationRef: `support:requester-message:${input.correlationId}`,
  };
}

export function verifySupportRequesterMessageConfirmation(input: {
  ...
  if (
    ...
    || !/^support:requester-message:[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(confirmation.confirmationRef)
  ...
  return confirmation as SupportRequesterMessageConfirmation;
}
