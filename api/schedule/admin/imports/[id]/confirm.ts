...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  return handleApi(res, async () => {
    ...
    if (["uploaded", "quarantined", "processing", "review", "approved", "active"].includes(source.status)) {
    ...
    const confirmed = await db.transaction(async (tx) => {
      const rows = await tx
        ...
        .where(
          and(
            eq(scheduleSourceVersions.id, id),
            eq(scheduleSourceVersions.institutionId, context.institutionId),
            inArray(scheduleSourceVersions.status, ["reserved"])
          )
        )
        .returning();
      if (!rows[0]) return [];
      await tx.insert(scheduleAudit).values({
        ...
        actorId: context.user.id,
        summary: { mimeType: source.mimeType, sizeBytes: source.sizeBytes },
      });
      return rows;
    });
