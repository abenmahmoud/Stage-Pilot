...
export default function ScheduleIcalReviewPanel({ importId, onApplied }: { importId: string; onApplied: () => void }) {
  ...
  useEffect(() => {
    ...
    async function read() {
      try {
        ...
        if (!cancelled) setError(reason instanceof Error ? reason.message : 'Lecture indisponible.');
      }
    }
    void read();
  ...
  const canValidate = !!parseIcalDecisions({ decisions: selected });
  ...
  return <div className="space-y-4 rounded-xl border border-slate-200 bg-white p-4 sm:p-6">
    ...
    {review && <>
      <p role="status" className="text-sm text-slate-600">{review.calendars.filter(c => c.status === 'applied').length} / {review.calendars.length} calendriers vérifiés et enregistrés{review.calendars.some(c => c.status === 'queued') ? ' · Enregistrement en cours…' : ''}</p>
      ...
      <button type="button" disabled={busy || review.sourceStatus !== 'review'} className="rounded-md border px-3 py-2 text-sm font-semibold" onClick={() => setDrafts(current => {
        const next = { ...current };
        for (const c of review.calendars) if (c.status === 'pending' && c.matchStatus === 'exact' && c.suggestedRef) next[c.id] = { id: c.id, decision: 'include', subjectRef: c.suggestedRef };
        return next;
      })}>Sélectionner les correspondances exactes</button>
      <div className="max-h-[32rem] divide-y overflow-y-auto">
        {review.calendars.map(c => {
      ...
      <div className="flex flex-wrap items-center gap-3 border-t pt-4">
        ...
        <p className="text-xs text-slate-500">La version doit ensuite être approuvée et activée.</p>
      </div>
    </>}
  </div>;
}
