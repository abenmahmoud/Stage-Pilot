...
const EMPTY: Draft = {
  ...
  featured: false, metaTitle: "", metaDescription: "", publishAt: "", expiresAt: "", assets: [], calendarEvents: [],
...
export default function ContentManagerPage() {
  ...
  async function act(action: "submit_review" | "publish" | "archive" | "duplicate" | "restore" | "verify_source" | "apply_editorial_corrections", version?: number) {
    if (!draft.id) return;
    const question = action === "publish" ? "Publier cette version sur le site ?" : action === "archive" ? "Archiver et retirer ce contenu du site ?" : action === "restore" ? `Restaurer la version ${version} ?` : action === "verify_source" ? "Confirmer que le texte, les dates, les liens et les fichiers ont été vérifiés ?" : action === "apply_editorial_corrections" ? "Appliquer uniquement les corrections automatiques sûres et conserver ce contenu en brouillon à vérifier ?" : null;
