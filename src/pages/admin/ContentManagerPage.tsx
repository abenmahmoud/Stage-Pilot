...
import { LEGACY_EDITORIAL_CORRECTIONS_UI_ENABLED } from "../../lib/feature-flags";
import { supabase } from "../../lib/supabase-browser";
import { PublicContentMarkdown } from "../../components/PublicContentMarkdown";
import { NewsPhoto } from "../../components/NewsPhoto";
import { ContentAudienceEditor } from "../../components/ContentAudienceEditor";
import type { ContentTargeting } from "../../../shared/content-targeting";
...
const EMPTY: Draft = {
  ...
  featured: false, metaTitle: "", metaDescription: "", publishAt: "", expiresAt: "", assets: [], calendarEvents: [], targeting: null,
...
function Preview({ draft, mobile, assets }: { draft: Draft; mobile: boolean; assets: Asset[] }) {
  const photoAssets = draft.assets.flatMap(link => {
    const asset = assets.find(candidate => candidate.id === link.assetId);
    return asset ? [{ ...asset, role: link.assetRole, signedUrl: asset.signedUrl || asset.url }] : [];
  });
  return (
    <article className={`content-markdown bg-white ${mobile ? "px-5 py-7" : "mx-auto max-w-3xl px-8 py-10"}`}>
      {draft.contentType !== "page" ? <NewsPhoto content={{ ...draft, assets: photoAssets }} className="!mb-6 aspect-[2/1] rounded-xl" /> : null}
...
export default function ContentManagerPage() {
  ...
  async function openItem(id: string) {
    ...
    try {
      ...
      const nextDraft: Draft = {
        ...
        calendarEvents: item.calendarEvents ?? [], targeting: item.targeting ?? null,
  ...
  async function save() {
    ...
    try {
      ...
      await load(result.itemId); setNotice("Brouillon enregistré. La version précédente est conservée.");
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Enregistrement impossible"); }
    finally { setBusy(false); }
  }

  async function act(action: "submit_review" | "publish" | "archive" | "duplicate" | "restore" | "verify_source" | "apply_editorial_corrections", version?: number) {
    if (!draft.id) return;
    if (draftDirty && ["publish", "submit_review"].includes(action)) { setError("Enregistrez vos modifications avant de faire valider ou publier cette information."); return; }
    const question = action === "publish" ? draft.audience === "tous" ? "Publier cette version pour tout le monde sur le site ?" : "Publier cette version dans l’espace des profils et classes choisis ? Aucun message automatique ne sera envoyé." : action === "archive" ? "Archiver et retirer ce contenu du site ?" : action === "restore" ? `Restaurer la version ${version} ?` : action === "verify_source" ? "Confirmer que le texte, les dates, les liens et les fichiers ont été vérifiés ?" : action === "apply_editorial_corrections" ? "Appliquer uniquement les corrections automatiques sûres et conserver ce contenu en brouillon à vérifier ?" : null;
    if (question && !window.confirm(question)) return;
    setBusy(true); setError("");
    try {
      const expectedVersion = items.find((item) => item.id === draft.id)?.version;
      if (action === "apply_editorial_corrections" && !expectedVersion) throw new Error("La version du brouillon est introuvable. Rechargez la page.");
      const command = action === "apply_editorial_corrections"
        ? { action, expectedVersion, confirmation: "CORRIGER" }
        : { action, version, ...(action === "publish" ? { expectedVersion } : {}) };
      const response = await apiFetch<unknown>(`content/admin/${draft.id}/action`, { method: "POST", body: JSON.stringify(command) });
      ...
      setNotice(action === "publish" ? "Contenu publié." : action === "submit_review" ? "Contenu envoyé à la direction pour validation." : action === "verify_source" ? "Reprise vérifiée. Le contenu peut maintenant être publié." : action === "apply_editorial_corrections" ? "Corrections sûres appliquées. Le brouillon reste à vérifier avant publication." : "Action terminée.");
