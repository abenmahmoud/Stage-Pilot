...
export function SupportReplySuggestion({ publicCode, revision, disabled, hasDraft, onApply }: {
  ...
  return <aside className="lycee-reply-suggestion" aria-label="Proposition pour ce dossier" aria-busy={loading}>
    ...
    {suggestion ? <>
      {!used ? <><p className="lycee-suggestion-context"><strong>{suggestion.title}</strong><small>{suggestion.facts[0]}</small></p><details className="lycee-suggestion-draft" open={replace || undefined}><summary>Voir le texte proposé</summary><p className="lycee-suggestion-preview" aria-label="Texte proposé à envoyer">{suggestion.draft}</p></details>{replace ? <div className="lycee-suggestion-replace" role="group" aria-label="Remplacer le brouillon"><p>Vous avez déjà commencé une réponse. Voulez-vous la remplacer ?</p><button type="button" disabled={disabled} onClick={() => setReplace(false)}>Garder mon brouillon</button><button type="button" disabled={disabled} onClick={() => { if (onApply(suggestion.draft)) { setUsed(true); setReplace(false); } }}>Remplacer par la proposition</button></div> : <button className="lycee-suggestion-apply" type="button" disabled={disabled} onClick={() => { if (hasDraft) { setReplace(true); return; } if (onApply(suggestion.draft)) setUsed(true); }}><Check aria-hidden="true" /> Préparer cette réponse</button>}<small>Le texte reste modifiable. Rien n’est envoyé avant « Valider et envoyer ».</small></> : <p>La réponse est prête dans le brouillon ci-dessous. Relisez-la, adaptez-la si nécessaire, puis envoyez-la.</p>}
      <details className="lycee-suggestion-sources"><summary>Informations vérifiées et sources</summary><ul>{suggestion.facts.map(fact => <li key={fact}>{fact}</li>)}</ul><p>{suggestion.sources.join(' · ')}</p><small>Consulté le {new Date(suggestion.checkedAt).toLocaleString('fr-FR', { timeZone: 'Europe/Paris', dateStyle: 'short', timeStyle: 'short' })}. Aucun code d’accès n’est inséré dans cette réponse.</small></details>
    </> : null}
  </aside>;
}
