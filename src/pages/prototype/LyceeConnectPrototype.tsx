...
function ConnectedAgentView({ onBack }: { onBack: () => void }) {
  ...
  const agentAttachmentDisabledReason = !selected
    ? null
    : selected.status === "clos"
      ? "Le dossier est clôturé. Rouvrez-le avant d’ajouter un document."
      : requiresSafeIdentityReply
        ? "Pièce jointe bloquée : vérifiez la personne, puis choisissez « Identité confirmée dans la liste » dans le bloc ci-dessus."
        : saving
          ? "Une opération est en cours. Attendez sa confirmation avant d’ajouter un document."
          : agentUploading
            ? "Le document sélectionné est en cours de dépôt et de contrôle."
            : agentDraftAttachments.length >= MAX_SUPPORT_FILES
              ? "Cinq documents sont déjà préparés. Retirez-en un ou envoyez la réponse avant d’en ajouter un autre."
              : null;
...
<section className="lycee-agent-identity" data-sensitive={supportReplyRequiresSchoolIdentity(selected)}><BadgeCheck aria-hidden="true" /><span><strong>{identityStatusLabels[selected.identityStatus]}</strong><small>{supportReplyRequiresSchoolIdentity(selected) ? "Demande sensible : vérifiez l’identité scolaire avant de transmettre des informations ou documents personnels." : "Adaptez le contrôle au niveau de sensibilité de la réponse."}</small></span><select aria-label="Niveau de vérification de l’identité" value={selected.identityStatus} disabled={saving} onChange={(event) => { const identityStatus = event.target.value as IdentityStatus; const identityMethod = identityStatus === "identite_confirmee" ? "official_roster" : identityStatus === "contact_verifie" ? (detail.contacts.some((contact) => contact.channel === "email" && contact.isVerified) ? "email_magic_link" : "phone_callback") : undefined; void updateRequest({ identityStatus, identityMethod }); }}><option value="non_verifiee">Coordonnées déclarées</option><option value="contact_verifie">Contact vérifié</option><option value="identite_confirmee">Identité confirmée dans la liste</option></select></section>
              ...
              <section className="lycee-reply-box">
                ...
                {agentAttachmentDisabledReason ? <small id="lycee-agent-attachment-help" className="lycee-agent-attachment-help" role="status"><CircleAlert aria-hidden="true" />{agentAttachmentDisabledReason}</small> : null}
                ...
                <div>{requiresSafeIdentityReply || !access?.canManageTemplates ? null : <button className="lycee-secondary-action" type="button" disabled={selected.status === "clos"} onClick={() => setShowTemplateSave((current) => !current)}><BookOpenCheck aria-hidden="true" /> Modèle</button>}<button className="lycee-secondary-action" type="button" aria-describedby={agentAttachmentDisabledReason ? "lycee-agent-attachment-help" : undefined} disabled={agentAttachmentDisabledReason !== null} onClick={() => agentFileInputRef.current?.click()}><Paperclip aria-hidden="true" /> {agentUploading ? "Contrôle…" : "Joindre un document"}</button><button className="lycee-primary-action" type="button" disabled={saving || agentUploading || translating || translationNeedsDecision || selected.status === "clos" || (!requiresSafeIdentityReply && !reply.trim())} onClick={() => void sendAgentReply()}><Send aria-hidden="true" /> {saving ? "Enregistrement…" : translatedReplyReady && translationDraft ? `Valider et envoyer en ${translationDraft.targetLanguage}` : "Valider et envoyer"}</button></div>
              </section>
            </>
