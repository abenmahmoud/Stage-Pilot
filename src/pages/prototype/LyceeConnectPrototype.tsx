...
import { routeSupportRequest } from "../../../shared/support-routing";
import {
...
const CONTACT_COLLECTION_DESCRIPTION =
  "Je souhaite faire vérifier et ajouter ou mettre à jour mes coordonnées personnelles dans l’annuaire du lycée.";
const CONTACT_REMOVAL_DESCRIPTION =
  "Je demande le retrait de mes coordonnées personnelles de l’annuaire du lycée.";
...
const agentCategoryLabels: Record<SupportCategory, string> = {
  inscription: "Inscriptions",
  affectation_classe: "Classes et emplois du temps",
...
async function uploadSupportFile(
  ...
  if (!isRequesterSupportUploadReservationPayload(reservation)) {
    throw new Error("Le document ne peut pas être préparé. Réessayez.");
  }
  ...
  if (!isSupportAttachmentConfirmationPayload(confirmation, reservation.attachment.id)) {
    throw new Error("L’envoi du document n’a pas pu être confirmé. Actualisez le dossier avant de réessayer.");
  }
  await completeRequesterUploadSubmission(submission);
  return reservation.attachment.id;
}
...
const services = [
  ...
  },
  {
    ...
    detail: "ENT, session du lycée ou messagerie académique",
    icon: KeyRound,
    tone: "gold",
  ...
  {
    title: "Cantine et intendance",
    detail: "Inscription, badge ou paiement de la cantine",
    icon: Utensils,
    tone: "gold",
    ...
    readApiResponse<unknown>(
      ...
      .then((payload) => {
        if (!isSupportMagicAccessPayload(payload)) {
          throw new Error("L’ouverture de votre accès n’a pas pu être confirmée. Réessayez.");
        }
        ...
        <div className={`lycee-content${IDENTITY_DEVICE_ACCESS_ENABLED ? " has-personal-home" : ""}`}>
          ...
          <section className="lycee-assistant" aria-labelledby="lycee-assistant-title">
            ...
            <div className="lycee-trust-row">
              ...
              <span><Users aria-hidden="true" /> Identité vérifiée avant tout accès personnel</span>
            </div>
          </section>

          <section className="lycee-core-tools" aria-label="Outils principaux du lycée">
            <button type="button" data-tool="news" onClick={() => changeView("news")}><span><Newspaper aria-hidden="true" /></span><div><strong>À la une</strong><small>Actualités, événements et documents utiles</small></div><em>Consulter <ChevronRight aria-hidden="true" /></em></button>
          ...
          <section className="lycee-services" aria-labelledby="lycee-services-title">
            <div className="lycee-section-title">
              <div>
                ...
                <h2 id="lycee-services-title">Les démarches courantes</h2>
              </div>
          ...
          <section className="lycee-lower-grid">
            <article className="lycee-news">
              ...
              <p>Accédez aux outils de suivi des stages et du Grand Oral avec le compte fourni par le lycée.</p>
              <a href={LYCEEGEST_URL}>Ouvrir LyceeGest <ChevronRight aria-hidden="true" /></a>
            </article>
            <article className="lycee-status-panel">
              <a href={ENT_URL} target="_blank" rel="noreferrer" title="Ouvrir l’ENT Monlycée.net dans un nouvel onglet">
          ...
          <section className="lycee-formations-band">
            <div><span className="lycee-eyebrow">Lycée polyvalent</span><h2>Général, technologique et professionnel</h2><p>Des parcours en CAP, en voie générale, technologique et professionnelle.</p></div>
  ...
  return (
    <div className="lycee-page lycee-news-page">
      <PageIntro
        ...
        onBack={onBack}
      />
      <div className="lycee-news-scope" role="group" aria-label="Période de publication">
      ...
      {!loading && !error && !selected ? (
        <section className="lycee-news-empty">
          <Newspaper aria-hidden="true" />
          <h2>{items.length > 0 ? "Aucun résultat pour cette recherche" : scope === "expired" ? "Aucune archive disponible" : "Les prochaines informations seront publiées ici"}</h2>
          <p>{items.length > 0 ? "Modifiez votre recherche ou choisissez une autre catégorie." : scope === "expired" ? "Les anciennes publications encore consultables sont regroupées dans les archives." : "Les formations et la présentation du lycée restent disponibles dans « Vie du lycée »."}</p>
  ...
  return (
    <div className="lycee-page lycee-trust-page">
      <PageIntro
        ...
        description="Comprendre les informations demandées et protéger l’accès à vos démarches."
        onBack={onBack}
      />
      ...
      <section className="lycee-trust-lead" aria-labelledby="trust-lead-title">
        ...
        <div>
          <span className="lycee-eyebrow">Phase pilote</span>
          <h2 id="trust-lead-title">Des informations limitées au traitement de votre demande</h2>
          <p>Le service est en phase pilote. Tant que son utilisation avec des données réelles n’a pas été validée par la direction et le délégué à la protection des données, utilisez uniquement des informations fictives pour les essais.</p>
        </div>
      </section>
      ...
      <div className="lycee-trust-grid">
        <article>
          ...
          <div><h2>Informations demandées</h2><p>Votre prénom, votre nom, un email ou un téléphone, puis les précisions utiles à votre demande. Joignez uniquement les documents nécessaires.</p></div>
        </article>
        <article>
          <span><KeyRound aria-hidden="true" /></span>
          <div><h2>Gardez vos codes confidentiels</h2><p>Ne communiquez aucun mot de passe, code d’activation ou code de vérification dans le chat ou une pièce jointe. Saisissez les codes de vérification uniquement dans le champ sécurisé prévu à cet effet.</p></div>
        </article>
        <article>
        ...
        <article>
          ...
          <div><h2>Suivi protégé</h2><p>Sur votre appareil, retrouvez les échanges dans « Mes demandes ». Sur un appareil partagé, fermez cet accès en partant. L’email permet de retrouver votre dossier ailleurs.</p></div>
        </article>
        <article>
          <span><Clock3 aria-hidden="true" /></span>
          <div><h2>Conservation encadrée</h2><p>Les durées de conservation et la procédure pour exercer vos droits restent à finaliser avec la direction et le délégué à la protection des données.</p></div>
        </article>
      </div>
      ...
      <section className="lycee-trust-contact" aria-labelledby="trust-contact-title">
        ...
        <div className="lycee-trust-actions">
          <a href="mailto:dpd@ac-creteil.fr">Écrire au délégué <Mail aria-hidden="true" /></a>
...
function localAssistantFallback(messages: AssistantChatMessage[], files: File[]): AssistantInsight {
  ...
  return {
    reply: policy.deterministicReply ?? laptopIntake?.reply ?? (informationIntent === "opening_hours" ? MISSING_OPENING_HOURS_REPLY
      ...
      : scheduleQuestion
      ? "Votre emploi du temps personnel nécessite une identité vérifiée et une version à jour. La consultation est momentanément indisponible. Vous pouvez réessayer ou préparer une demande ici."
      : readyToCreate
        ? `Votre demande concerne « ${label} ». Complétons les informations utiles ici. Vous pourrez relire et modifier votre demande avant de confirmer l’envoi.`
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (payload.status === "choose_contact" && Array.isArray(payload.options) && payload.options.length > 0) {
          ...
          setError("La recherche prend plus de temps que prévu. Réessayez dans quelques instants.");
          setState("unavailable");
        }
      } catch {
        if (active) {
  ...
  return (
    <section className="lycee-identity-device" aria-labelledby="identity-device-title">
      ...
      <div className="lycee-identity-device-copy">
        <strong id="identity-device-title">{state === "identify" ? "Qui êtes-vous ?" : state === "choosing_contact" ? "Où souhaitez-vous recevoir votre code ?" : state === "awaiting_code" ? "Quel code avez-vous reçu ?" : "Vérifions votre accès"}</strong>
        <p>{state === "identify" ? "Indiquez votre identité telle qu’elle est enregistrée au lycée. Si vous êtes parent, indiquez votre propre nom." : state === "choosing_contact" ? "Choisissez un seul moyen parmi les coordonnées connues du lycée." : state === "awaiting_code" ? "Saisissez le code reçu pour confirmer votre identité." : "Vos informations personnelles restent protégées."}</p>
      </div>
      {state === "identify" ? (
        ...
        <IdentityContactChoices options={contactOptions} busy={busy} onSelect={chooseContact} onCorrect={requestContactCorrection}
          onIdentify={() => { setContactOptions([]); setState('identify'); setError(null); }} />
        ...
        <form onSubmit={verifyCode}>
          ...
          <button type="submit" disabled={busy || code.length !== 6}>{busy ? "Vérification…" : "Vérifier"}</button>
...
function HelpDeskView({
    ...
    void readSupportDeviceDraft<AssistantInsight>().then((draft) => {
      ...
      setDraftNotice(
        draft.hadAttachments
          ? "Votre brouillon a été retrouvé. Pour votre sécurité, sélectionnez de nouveau les documents à joindre."
          : "Votre brouillon a été retrouvé sur cet appareil."
      );
    }).finally(() => {
  ...
  function selectFiles(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (invalid) {
      setSubmitError("Formats acceptés : PDF, image, texte, Word ou Excel. Taille maximale : 10 Mo par fichier.");
      event.target.value = "";
      return;
    }
  ...
  async function submitRequest(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!isSupportRequestCreationPayload(
        ...
        throw new Error("L’enregistrement de votre demande n’a pas pu être confirmé. Réessayez sans modifier les informations.");
      }
  ...
  if (ticketCode) {
    return (
      <div className="lycee-page lycee-confirmation-view">
        ...
        <p>Votre demande est enregistrée. Retrouvez les réponses du lycée et les documents dans « Mes demandes ».</p>
        ...
        <div className="lycee-confirmation-note"><Smartphone aria-hidden="true" /><span>Ouvrez « Mes demandes » sur cet appareil. Conservez votre numéro de dossier ; il ne suffit pas, à lui seul, à donner accès à la demande.</span></div>
        {confirmationChannel === "email"
          ? <div className="lycee-confirmation-note"><Mail aria-hidden="true" /><span>L’envoi d’un email de suivi est prévu à l’adresse indiquée. Il permet de retrouver le dossier sur un autre appareil. Pensez à vérifier les courriers indésirables.</span></div>
    ...
    <div className="lycee-page">
      <PageIntro
        ...
        description={initialContactCollection
          ...
          : "Posez votre question ou expliquez votre démarche. Nous vous accompagnons ici."}
        onBack={onBack}
      />
        ...
        <div className="lycee-chat-workspace">
          ...
          {insight?.scope === "safescol" ? (
            <div className="lycee-safescol-direction" role="status">
              ...
              {SAFESCOL_ACCESS_ENABLED && SAFESCOL_URL
                ...
                : <em>Le lien SafeScol n’est pas encore disponible sur ce site. Adressez-vous à un adulte du lycée.</em>}
            </div>
          ) : null}

          {conversationStopped && !canCreateRequest && insight?.scope !== "safescol" ? (
            <div className="lycee-chat-next">
              <button className="lycee-primary-action" type="button" onClick={restartConversation}>Commencer une demande au lycée</button>
            </div>
          ) : null}
          ...
          {showDetails && !classicForm && !initialContactCollection ? <ChatRequestIntake profile={profile} setProfile={value => setProfile(value as RequesterProfile)} values={formValues} update={updateFormValue} description={conversationDescription} category={selectedCategory?.label ?? "Demande au lycée"} fileNames={files.map(file => file.name)} busy={submitting} reuseIdentityDetails={reuseIdentityDetails} error={submitError} onSubmit={submitRequest} onBack={() => setShowDetails(false)} onAttach={() => fileInputRef.current?.click()} /> : showDetails ? (
            <form ref={caseFormRef} className="lycee-case-form" onSubmit={submitRequest}>
              ...
              <div className="lycee-case-security"><BadgeCheck aria-hidden="true" /><span><strong>Vérification adaptée à la demande</strong><small>{initialContactCollection ? "Le lien de suivi confirme l’accès à votre email. Le lycée vérifie ensuite la modification demandée à partir de son annuaire." : "Le lien de suivi donne accès à votre dossier. Pour consulter des informations personnelles, confirmez votre identité dans le chat avec un contact connu du lycée."}</small></span></div>
...
const agentStatusLabels: Record<string, string> = {
  ...supportStatusLabels,
  attente_demandeur: "En attente de votre réponse",
...
function ConnectedRequestsView({ ticketCode, onBack, accessLinkError }: { ticketCode: string | null; onBack: () => void; accessLinkError?: string | null }) {
  ...
  async function selectFollowupFiles(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (invalid) {
      setError("Formats acceptés : PDF, image, texte, Word ou Excel. Taille maximale : 10 Mo par fichier.");
      return;
    }
  ...
  async function sendReply(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (followupFiles.length > 0) {
        ...
        if (failedCount > 0) {
          setError(`${failedCount} document${failedCount > 1 ? "s n’ont pas été joints" : " n’a pas été joint"}. Réessayez : les documents déjà reçus ne seront pas renvoyés.`);
          return;
        }
        setFollowupFiles([]);
      }
    ...
    <div className="lycee-page">
      ...
      <div className="lycee-shared-device-action">
        ...
        <div className="lycee-device-action-buttons">
          ...
          <button type="button" disabled={forgettingDevice} onClick={() => void forgetThisDevice()}><LogOut aria-hidden="true" />{forgettingDevice ? "Fermeture…" : "Fermer l’accès sur cet appareil"}</button>
        </div>
      </div>
      {error ? <div className="lycee-form-error" role="alert"><CircleAlert aria-hidden="true" />{error}</div> : null}
      {detailError && selectedCode ? <div className="lycee-form-error" role="alert"><CircleAlert aria-hidden="true" /><span>{detailError}</span><button type="button" disabled={detailLoading} onClick={() => void loadDetail(selectedCode)}>{detailLoading ? "Nouvel essai…" : "Recharger le dossier"}</button></div> : null}
...
function DemoRequestsView({ ticketCode, onBack }: { ticketCode: string | null; onBack: () => void }) {
  ...
  function sendDemoReply(event: React.FormEvent<HTMLFormElement>) {
    ...
    setDemoMessages((current) => [
      ...
      { id: crypto.randomUUID(), direction: "is-agent", author: "Assistant Blaise", date: "Maintenant", body: "Merci, votre précision est ajoutée au dossier. La personne qui traite la demande pourra la consulter." },
    ]);
    setDemoReply("");
  }
...
function ServicesView({ onHelp, onCollect, onBack }: { onHelp: (prompt?: string) => void; onCollect: () => void; onBack: () => void }) {
  const serviceGroups = [
    ...
    { title: "Matériel du lycée", description: "Professeurs : signalez un équipement de salle, suivez le dossier et consultez les prochains passages SPIE.", icon: Wrench, color: "green", progress: "Signalement et suivi en ligne", action: "Ouvrir l’espace matériel", href: "/materiel" },
    ...
    { title: "Cantine et intendance", description: "Inscription à la cantine, badge, repas ou paiement : adressez votre demande à l’intendance.", icon: Utensils, color: "gold", progress: "Service responsable : intendance", action: "Poser ma question", help: true, prompt: "J’ai une question sur la cantine pour le service d’intendance." },
    { title: "Rendez-vous au lycée", description: SCHOOL_PUBLIC_INFORMATION.welcome, icon: CalendarDays, color: "blue", progress: "Rendez-vous à confirmer par le lycée", action: "Demander un rendez-vous", help: true, prompt: "Je souhaite demander un rendez-vous au lycée." },
    { title: "ENT Monlycée.net", description: "Ouvrez votre espace numérique avec vos identifiants personnels.", icon: KeyRound, color: "blue", progress: "Accès à Monlycée.net", action: "Accéder à l’ENT", href: ENT_URL, external: true },
    ...
    { title: "Coordonnées personnelles", description: "Demandez l’ajout, la correction ou le retrait d’un email ou d’un téléphone.", icon: UserRound, color: "gold", progress: "Modification validée par le lycée", action: "Demander une modification", collect: true },
    { title: "LyceeGest", description: "Stages, Grand Oral et outils de gestion du lycée", icon: BarChart3, color: "blue", progress: "Compte du lycée nécessaire", action: "Ouvrir LyceeGest", href: LYCEEGEST_URL },
  ...
  return (
    <div className="lycee-page">
      <PageIntro eyebrow="Application du lycée" title="Mes services" description="Retrouvez les accès utiles et choisissez la démarche adaptée à votre besoin." onBack={onBack} />
      <div className="lycee-services-catalog">
        ...
        {serviceGroups.map((service) => (
          <article data-tone={service.color} key={service.title}>
            ...
            {service.help ? <button type="button" onClick={() => onHelp(service.prompt)}>{service.action}<ChevronRight aria-hidden="true" /></button> : service.collect ? <button type="button" onClick={onCollect}>{service.action}<ChevronRight aria-hidden="true" /></button> : <a href={service.href} target={service.external ? "_blank" : undefined} rel={service.external ? "noreferrer" : undefined}>{service.action}{service.external ? <ExternalLink aria-hidden="true" /> : <ChevronRight aria-hidden="true" />}</a>}
          </article>
        ))}
      </div>
      <section className="lycee-mail-bridge">
        <div><Headphones aria-hidden="true" /><span><span className="lycee-eyebrow">Besoin d’aide</span><h2>Une demande, un suivi</h2><p>Expliquez votre besoin, joignez les documents utiles et retrouvez les échanges dans « Mes demandes ».</p></span></div>
...
function SchoolView({ onBack, onHelp }: { onBack: () => void; onHelp: (prompt?: string) => void }) {
  ...
  const formations = [
    ...
    },
    {
      ...
      items: "STL : sciences physiques et chimiques en laboratoire. STMG : gestion et finance, mercatique, ressources humaines et communication.",
    },
    {
      title: "Voie professionnelle",
      icon: Settings2,
      description: "Des formations concrètes dans l’énergie, les procédés et la maintenance.",
      items: "Secondes professionnelles MTNE et MPMIA, puis baccalauréats professionnels MELEC et PCEPC.",
    },
    {
      title: "CAP",
  ...
  const detailedPrograms = [
    ...
    },
    {
      ...
      path: "Seconde de la famille MTNE · baccalauréat professionnel en trois ans",
    },
    {
      family: "Voie professionnelle",
      title: "PCEPC",
      ...
      path: "Seconde de la famille MPMIA · baccalauréat professionnel en trois ans",
    },
    {
  ...
  const schoolLife = [
    {
      ...
      detail: "Consultez le CDI pour connaître ses horaires d’accueil.",
    ...
    },
    {
      ...
      detail: "Pour les activités, les horaires et l’inscription de cette année, renseignez-vous auprès de l’équipe d’EPS.",
    ...
    },
    {
      ...
      detail: "Les dates et les modalités d’accueil doivent être confirmées par les équipes responsables.",
  ...
  return (
    <div className="lycee-page">
      <PageIntro eyebrow="Lycée polyvalent" title="Blaise Cendrars, Sevran" description="Formations, vie scolaire, rendez-vous et coordonnées : retrouvez les repères utiles pour votre parcours au lycée." onBack={onBack} />
      ...
      <SchoolParentsMeeting />
      <div className="lycee-school-stats"><div><strong>Polyvalent</strong><span>général, technologique et professionnel</span></div><div><strong>8</strong><span>spécialités générales</span></div><div><strong>CAP au bac</strong><span>parcours de formation</span></div><div><strong>Euro</strong><span>section européenne en anglais</span></div></div>
      <nav className="lycee-school-nav" aria-label="Rubriques du lycée">
        ...
        <Link to="/?view=school#infos-pratiques">Informations pratiques</Link>
      </nav>
      ...
      <section className="lycee-practical" id="infos-pratiques" aria-labelledby="practical-title">
        ...
        <div className="lycee-practical-grid">
          <a href="https://maps.app.goo.gl/qoEq5cf4UwTm5diC7" target="_blank" rel="noreferrer"><MapPin aria-hidden="true" /><span><strong>{SCHOOL_PUBLIC_INFORMATION.address}</strong><small>{SCHOOL_PUBLIC_INFORMATION.locality} · ouvrir l’itinéraire</small></span><ExternalLink aria-hidden="true" /></a>
          <a href={SCHOOL_PUBLIC_INFORMATION.phoneHref}><Phone aria-hidden="true" /><span><strong>{SCHOOL_PUBLIC_INFORMATION.phone}</strong><small>Accueil du lycée</small></span><ChevronRight aria-hidden="true" /></a>
...
async function fetchAgentRequestDetail(code: string): Promise<AgentRequestDetail> {
  ...
  if (!isAgentRequestDetail(payload)) {
    throw new Error("Le détail de la demande n’a pas pu être chargé.");
  }
  return payload;
}
...
function ConnectedAgentView({ onBack }: { onBack: () => void }) {
  ...
  const managementPanelRef = useRef<HTMLDetailsElement>(null);
  const historyPanelRef = useRef<HTMLDetailsElement>(null);
  const notesPanelRef = useRef<HTMLDetailsElement>(null);
  const replyFieldRef = useRef<HTMLTextAreaElement>(null);

  function openWorkSection(section: SupportWorkSection) {
    if (section === "reply") {
      replyFieldRef.current?.focus();
      return;
    }
    const panel = section === "management" ? managementPanelRef.current
      : section === "history" ? historyPanelRef.current : notesPanelRef.current;
    if (!panel) return;
    panel.open = true;
  ...
  async function selectAgentFiles(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (acceptedEntries.length !== files.length || acceptedEntries.length === 0) {
      setError("Vous pouvez joindre au maximum 5 documents à une réponse.");
  ...
  const latestMessage = latestRequesterMessage(detail?.messages ?? []);
  ...
  const emailContact = detail?.contacts.find((contact) => contact.channel === "email") ?? null;
  const personalContactAction = selected?.subcategory === "contact_personnel_ajout"
    ? "Ajouter ou corriger l’adresse personnelle"
    : selected?.subcategory === "contact_personnel_retrait"
      ? "Retirer l’adresse personnelle"
      : null;
  ...
  const expectedRoute = selected ? routeSupportRequest({
    category: selected.category,
    subject: selected.subject,
    description: selected.description,
  }) : null;
  const suggestedTeamMismatch = Boolean(
    selected
    && expectedRoute
    && selected.assignedTeam !== expectedRoute.service
    && access?.canRoute
  ...
  return (
    <div className="lycee-page lycee-agent-page">
      ...
      <div className={`lycee-agent-workspace ${selectedCode ? 'has-selection' : ''}`}>
        ...
        <article className="lycee-agent-detail" aria-label="Détail de la demande" aria-busy={detailLoading}>
          {selectedCode ? <button className="lycee-agent-back-to-list" type="button" disabled={queueNavigationDisabled} onClick={() => setSelectedCode(null)}><ArrowLeft aria-hidden="true" /> Toutes les demandes</button> : null}
          {detailLoading ? <div className="lycee-loading-state" role="status" aria-live="polite"><Clock3 aria-hidden="true" /> Chargement du dossier…</div> : selected && detail ? (
            <>
              ...
              <SupportRequestOverview
                key={selected.publicCode}
                message={latestMessage?.bodyText ?? selected.description}
                messageDate={latestMessage ? supportDate(latestMessage.createdAt) : null}
                service={supportTeamLabel(selected.assignedTeam)}
                statusLabel={agentStatusLabels[selected.status] ?? selected.status}
                beneficiary={selected.beneficiaryType !== "self" ? [selected.beneficiaryFirstName, selected.beneficiaryLastName].filter(Boolean).join(" ") || null : null}
                routingPending={detail.routingReview?.status === "pending"}
                focus={{ status: selected.status, needsIdentity: requiresSafeIdentityReply, duplicatePending: detail.duplicateReview?.status === "pending", callbackPending: Boolean(activeCallback), assigned: Boolean(selected.assignedTo) }}
                onOpen={openWorkSection}
              />
              {suggestedTeamMismatch && expectedRoute ? <section className="lycee-agent-route-correction" aria-label="Correction du classement proposée"><ArrowRightLeft aria-hidden="true" /><span><small>Classement conseillé</small><strong>{supportTeamLabel(expectedRoute.service)}</strong><p>Le contenu de la demande correspond à ce service. Le transfert conserve les échanges et les documents.</p></span><button type="button" disabled={saving} onClick={() => void updateRequest({ assignedTeam: expectedRoute.service, status: ["nouveau", "a_qualifier"].includes(selected.status) ? "assigne" : selected.status })}>Transférer</button></section> : null}
              <section className="lycee-reply-box">
                <div>
                  <span><MessageCircleMore aria-hidden="true" /> Votre réponse</span>
                ...
                {selected.status !== 'clos' ? <SupportReplySuggestion key={selected.publicCode} publicCode={selected.publicCode} revision={selected.updatedAt} hasDraft={Boolean(reply.trim())} disabled={saving || translating}
                  onApply={(text) => { changeReply(text); replyFieldRef.current?.focus(); return true; }} /> : null}
                {requiresSafeIdentityReply ? <div className="lycee-reply-identity-help"><p>Identité à confirmer : vous pouvez déjà expliquer la démarche. Les codes et documents personnels restent protégés.</p><button className="lycee-secondary-action" type="button" disabled={saving || selected.status === "clos"} onClick={() => openWorkSection('management')}>Vérifier l’identité</button></div> : null}
                ...
                <div>{requiresSafeIdentityReply || !access?.canManageTemplates ? null : <button className="lycee-secondary-action" type="button" disabled={selected.status === "clos"} onClick={() => setShowTemplateSave((current) => !current)}><BookOpenCheck aria-hidden="true" /> Modèle</button>}<button className="lycee-secondary-action" type="button" disabled={selected.status === "clos" || requiresSafeIdentityReply || saving || agentUploading || agentDraftAttachments.length >= MAX_SUPPORT_FILES} onClick={() => agentFileInputRef.current?.click()}><Paperclip aria-hidden="true" /> {agentUploading ? "Vérification…" : "Joindre"}</button><span className="lycee-agent-send-help">L’envoi vous attribue le dossier.</span><button className="lycee-primary-action" type="button" disabled={saving || agentUploading || translating || translationNeedsDecision || selected.status === "clos" || !reply.trim() || generalReplyNeedsConfirmation} onClick={() => void sendAgentReply()}><Send aria-hidden="true" /> {saving ? "Enregistrement…" : translatedReplyReady && translationDraft ? `Valider et envoyer en ${translationDraft.targetLanguage}` : "Valider et envoyer"}</button></div>
              </section>
              ...
              <details key={selected.publicCode + "-managementPanelRef"} ref={managementPanelRef} className="lycee-agent-panel">
                <summary><strong>Coordonnées et traitement</strong><span>{supportTeamLabel(selected.assignedTeam)} · {identityStatusLabels[selected.identityStatus]}</span></summary>
                <div className="lycee-agent-panel-content">
              ...
              {phoneContact ? (
                <section className="lycee-agent-callback" data-status={activeCallback?.status ?? lastFinishedCallback?.status ?? "idle"}>
                  ...
                  <span>
                    ...
                    <strong>{activeCallback?.status === "todo" ? "Rappel à effectuer" : activeCallback?.status === "in_progress" ? "Rappel en cours" : lastFinishedCallback?.status === "done" ? "Dernier rappel terminé" : "Aucun rappel en attente"}</strong>
                  ...
                  <div>
                    ...
                    {activeCallback?.status === "todo" ? <button type="button" disabled={saving} onClick={() => void updateCallback(activeCallback.id, "claim")}>Prendre en charge le rappel</button> : null}
...
<section className="lycee-agent-identity" data-sensitive={supportReplyRequiresSchoolIdentity(selected)}><BadgeCheck aria-hidden="true" /><span><strong>{identityStatusLabels[selected.identityStatus]}</strong><small>{supportReplyRequiresSchoolIdentity(selected) ? "Demande sensible : vérifiez l’identité scolaire avant de transmettre des informations ou documents personnels." : "Adaptez le contrôle au niveau de sensibilité de la réponse."}</small></span><select aria-label="Niveau de vérification de l’identité" value={selected.identityStatus} disabled={saving} onChange={(event) => { const identityStatus = event.target.value as IdentityStatus; const identityMethod = identityStatus === "identite_confirmee" ? "official_roster" : identityStatus === "contact_verifie" ? (detail.contacts.some((contact) => contact.channel === "email" && contact.isVerified) ? "email_magic_link" : "phone_callback") : undefined; void updateRequest({ identityStatus, identityMethod }); }}><option value="non_verifiee">Coordonnées déclarées</option><option value="contact_verifie">Contact vérifié</option><option value="identite_confirmee">Identité confirmée dans la liste</option></select></section>
              {personalContactAction ? <section className="lycee-agent-contact-review" aria-label="Contrôle de la demande de coordonnées"><Mail aria-hidden="true" /><div><small>Action demandée</small><strong>{personalContactAction}</strong><p>{emailContact?.value ?? "Adresse email absente du dossier"}</p></div><dl><div data-ready={emailContact?.isVerified}><dt>Email</dt><dd>{emailContact?.isVerified ? "Adresse vérifiée par le lien" : "Lien de vérification attendu"}</dd></div><div data-ready={selected.identityStatus === "identite_confirmee"}><dt>Identité</dt><dd>{selected.identityStatus === "identite_confirmee" ? "Confirmée dans la liste officielle" : "Rapprochement officiel à terminer"}</dd></div></dl><p className="lycee-agent-contact-warning"><CircleAlert aria-hidden="true" /> Ne confirmez jamais une mise à jour du Webmail tant que son registre externe n’a pas renvoyé une preuve.</p></section> : null}
              {detail.duplicateReview ? <section className="lycee-agent-duplicate" data-status={detail.duplicateReview.status}><Copy aria-hidden="true" /><span><strong>{detail.duplicateReview.status === "pending" ? "Possible doublon à vérifier" : detail.duplicateReview.status === "confirmed" ? "Doublon confirmé" : "Dossiers distincts"}</strong><small>Même contact et même catégorie sur sept jours. Aucun dossier n’est fusionné automatiquement.</small></span><div>{detail.duplicateReview.candidatePublicCode ? <button type="button" onClick={() => setSelectedCode(detail.duplicateReview?.candidatePublicCode ?? null)}>Voir {detail.duplicateReview.candidatePublicCode}</button> : null}{detail.duplicateReview.status === "pending" && detail.duplicateReview.candidatePublicCode ? <><button type="button" disabled={saving} onClick={() => void updateRequest({ duplicateDecision: "dismissed" })}>Dossiers distincts</button><button type="button" disabled={saving} onClick={() => void updateRequest({ duplicateDecision: "confirmed" })}>Confirmer</button></> : detail.duplicateReview.status === "pending" ? <small>Validation réservée à un agent autorisé à consulter les deux dossiers.</small> : <small>Décision enregistrée dans l’historique.</small>}</div></section> : null}
                </div>
              </details>
              <details key={selected.publicCode + "-notesPanelRef"} ref={notesPanelRef} className="lycee-agent-panel">
                <summary><strong>Notes internes et clôture</strong><span>{selected.status === "clos" ? "Dossier clôturé" : "Suivi réservé aux agents"}</span></summary>
                <div className="lycee-agent-panel-content">
              <section className="lycee-agent-actions"><div><span><StickyNote aria-hidden="true" /><strong>Note interne</strong><small>Visible uniquement des agents.</small></span><textarea aria-label="Note interne" rows={3} value={internalNote} disabled={saving} onChange={(event) => changeInternalNote(event.target.value)} placeholder="Diagnostic, appel effectué ou prochaine action…" maxLength={5000} /><button type="button" disabled={saving || !internalNote.trim()} onClick={() => void saveInternalNote()}>Ajouter la note</button></div><div data-closed={selected.status === "clos"}><span><CheckCircle2 aria-hidden="true" /><strong>{selected.status === "clos" ? "Dossier clôturé" : "Clôturer la demande"}</strong><small>{selected.status === "clos" ? selected.subjectContext.closureReason ?? "Motif enregistré dans l’historique." : requiresSafeIdentityReply ? "Confirmez d’abord l’identité scolaire pour cette demande sensible." : "Indiquez le motif de clôture ; il sera conservé dans l’historique."}</small></span>{selected.status === "clos" ? <button type="button" disabled={saving} onClick={() => void updateRequest({ status: "en_cours" })}>Rouvrir le dossier</button> : <><textarea aria-label="Motif de clôture" rows={3} value={closureReason} onChange={(event) => changeClosureReason(event.target.value)} placeholder="Solution apportée ou raison de la clôture…" maxLength={500} /><button type="button" disabled={saving || !closureReason.trim() || requiresSafeIdentityReply} onClick={() => void updateRequest({ status: "clos", closureReason })}>Clôturer le dossier</button></>}</div></section>
                </div>
              </details>
            </>
