  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setFiche(data);
        if (data.statut && data.statut !== "brouillon") setStep(4);
      } catch {}
      try {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {step === 0 && !isPostSubmission && (
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-gray-500">Choisissez vos deux enseignements de spécialité</p>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Spécialité 1 *</label>
              <select
                value={spe1}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Spécialité 2 *</label>
              <select
                value={spe2}
      ...
      {step === 1 && !isPostSubmission && (
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-gray-500">Formulez une question problématisée en lien avec votre programme</p>
          </CardHeader>
          <CardContent className="space-y-5">
            <div className="rounded-xl bg-blue-50 border border-blue-200 p-3 text-sm text-blue-700">
              Les questions portent sur les deux enseignements de spécialité soit pris isolément, soit abordés de manière transversale (Note de service du 27-07-2021).
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Question 1 *</label>
              <textarea
                value={(fiche.question1 as string) || ""}
              ...
              />
              <p className="text-xs text-gray-400 mt-1 text-right">{((fiche.question1 as string) || "").length}/300</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Spécialité(s) concernée(s) pour Q1 *</label>
              <input
                type="text"
                value={(fiche.specialitesQuestion1 as string) || ""}
            ...
            <div>
              ...
              <textarea
                value={(fiche.question2 as string) || ""}
                onChange={(e) => update("question2", e.target.value)}
                maxLength={300} rows={3}
                className="w-full rounded-xl border border-gray-300 px-4 py-2.5 text-sm outline-none focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 resize-none"
                placeholder="Formulez votre deuxième question…"
              />
              <p className="text-xs text-gray-400 mt-1 text-right">{((fiche.question2 as string) || "").length}/300</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Spécialité(s) concernée(s) pour Q2 *</label>
              <input
                type="text"
                value={(fiche.specialitesQuestion2 as string) || ""}
      ...
      {step === 2 && !isPostSubmission && (
        <Card>
          <CardHeader>
            <h2 className="text-lg font-bold font-heading">Mes enseignants de spécialité</h2>
            <p className="text-sm text-gray-500">Sélectionnez les professeurs qui valideront votre fiche</p>
          </CardHeader>
          <CardContent className="space-y-4">
            <div>
              ...
              <select
                value={(fiche.profSpe1Id as string) ?? ""}
                onChange={(e) => update("profSpe1Id", e.target.value || null)}
                className="w-full rounded-xl border border-gray-300 px-4 py-2.5 text-sm outline-none focus:border-primary-500"
              >
                <option value="">Choisir…</option>
                {profs.map((p) => (
                  <option key={p.id} value={p.id}>
                    {p.nom} {p.prenom}{p.matieres ? ` — ${p.matieres}` : ""}
                  </option>
                ))}
              </select>
            </div>
            <div>
              ...
              <select
                value={(fiche.profSpe2Id as string) ?? ""}
                onChange={(e) => update("profSpe2Id", e.target.value || null)}
                className="w-full rounded-xl border border-gray-300 px-4 py-2.5 text-sm outline-none focus:border-primary-500"
              >
                <option value="">Choisir…</option>
                {profs.map((p) => (
      ...
      {step === 3 && !isPostSubmission && (
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-gray-500">Vérifiez vos informations avant soumission</p>
          </CardHeader>
          <CardContent className="space-y-5">
            <div className="rounded-xl bg-gray-50 p-4 space-y-4">
              <h3 className="text-sm font-semibold text-gray-900">Question 1</h3>
              <p className="text-sm text-gray-700">{(fiche.question1 as string) || "—"}</p>
              <p className="text-xs text-gray-500">Spécialités : {(fiche.specialitesQuestion1 as string) || "—"}</p>
            </div>
            <div className="rounded-xl bg-gray-50 p-4 space-y-4">
              <h3 className="text-sm font-semibold text-gray-900">Question 2</h3>
              <p className="text-sm text-gray-700">{(fiche.question2 as string) || "—"}</p>
              <p className="text-xs text-gray-500">Spécialités : {(fiche.specialitesQuestion2 as string) || "—"}</p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Numéro de candidat (optionnel)</label>
              <input
                type="text"
                value={(fiche.numeroCanditat as string) || ""}
            ...
            <div className="flex justify-between pt-2">
              ...
              >
                ...
                {saving ? "Envoi…" : "Soumettre à mes enseignants de spécialité"}
              </button>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex items-center gap-0">
              {[
                { label: "Saisie élève", done: true },
                { label: "Prof spé 1", done: ["valide_prof1", "soumis_prof2", "valide_prof2", "soumis_proviseur", "valide_proviseur", "finalise"].includes((fiche.statut as string) || "") },
                { label: "Prof spé 2", done: ["valide_prof2", "soumis_proviseur", "valide_proviseur", "finalise"].includes((fiche.statut as string) || "") },
                { label: "Chef d'établissement", done: ["valide_proviseur", "finalise"].includes((fiche.statut as string) || "") },
                { label: "Finalisé", done: fiche.statut === "finalise" },
              ].map((t, i) => (
                <div key={i} className="flex items-center gap-0 flex-1">
                  <div className="flex flex-col items-center gap-1 flex-1">
                    ...
                    <span className="text-[10px] text-gray-500 text-center leading-tight">{t.label}</span>
                  </div>
                  {i < 4 && (<div className={`h-0.5 flex-1 mt-[-18px] ${t.done ? "bg-green-500" : "bg-gray-200"}`} />)}
                </div>
              ))}
            </div>
            ...
            {fiche.commentaireProf1 && (
              <div className="rounded-xl bg-amber-50 border border-amber-200 p-4">
                <p className="text-xs font-semibold text-amber-700 mb-1">Remarque de votre enseignant (spé 1)</p>
                <p className="text-sm text-amber-800">{fiche.commentaireProf1 as string}</p>
              </div>
            )}

            {fiche.commentaireProf2 && (
              <div className="rounded-xl bg-amber-50 border border-amber-200 p-4">
                <p className="text-xs font-semibold text-amber-700 mb-1">Remarque de votre enseignant (spé 2)</p>
                <p className="text-sm text-amber-800">{fiche.commentaireProf2 as string}</p>
              </div>
            )}

            {fiche.statut === "finalise" && (
              <div className="rounded-xl bg-green-50 border border-green-200 p-4 space-y-3">
                <p className="text-sm text-green-800 font-medium">Votre fiche Grand Oral est finalisée !</p>
                <p className="text-sm text-green-700">Imprimez cette fiche et présentez-la le jour de votre Grand Oral.</p>
                <button className="inline-flex items-center gap-2 rounded-xl bg-green-600 px-4 py-2 text-sm font-semibold text-white hover:bg-green-700 transition-all">
                  <Download className="w-4 h-4" />Télécharger ma fiche PDF
                </button>
              </div>
            )}
          </CardContent>
