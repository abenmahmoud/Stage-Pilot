...
import { communicationInboundPreviewDatabaseUrl } from "../workers/communication-inbound-preview-target.mjs";
...
const [drizzle, { drizzle: createDatabase }, { default: postgres }, schema, inboundModule, parserModule] = await Promise.all([
  import("drizzle-orm"),
...
const client = postgres(databaseUrl, { prepare: false, max: 1, connect_timeout: 10, idle_timeout: 5,
  ssl: { rejectUnauthorized: true }, onnotice: () => {} });
try { await runRecipe(createDatabase(client, { schema })); }
finally { await client.end({ timeout: 5 }); }

async function runRecipe(db) {
